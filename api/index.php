...
define('NOTIFICATION_EMAIL_TO', getenv('FORMS_EMAIL_TO') ?: 'ciclis541@gmail.com');
define('NOTIFICATION_EMAIL_FROM', getenv('FORMS_EMAIL_FROM') ?: 'no-reply@mediatoryamal.ru');
...
{
    $encoded = base64_encode((string) $subject);
    return '=?UTF-8?B?' . $encoded . '?=';
}

function notification_subject($kind)
{
    if ($kind === 'appointment') {
        return 'Новая онлайн-запись с сайта';
    }

    return 'Новая заявка с сайта';
}

function notification_body($kind, $payload)
{
    $title = $kind === 'appointment' ? 'Тип: Онлайн-запись' : 'Тип: Обратная связь';
    $lines = [$title, 'Получено: ' . now_iso(), '', 'Данные заявки:'];
...
function send_submission_email($kind, $payload)
{
    $to = sanitize_header_value(NOTIFICATION_EMAIL_TO);
    if ($to === '') {
        return false;
    }

    $from = sanitize_header_value(NOTIFICATION_EMAIL_FROM);
    $headers = [
        'MIME-Version: 1.0',
    ...
    $subject = encode_mail_subject(notification_subject($kind));
    $body = notification_body($kind, $payload);
...
try {
    if ($path === '/leads' || $path === '/contact') {
        ...
        $email_sent = send_submission_email('lead', $data);
        respond(201, ['data' => $data, 'meta' => ['email_sent' => $email_sent]]);
    }

    if ($path === '/appointments') {
        $data = validate_appointment($payload);
        write_submission('appointment', $data);
        $email_sent = send_submission_email('appointment', $data);
        respond(201, ['data' => $data, 'meta' => ['email_sent' => $email_sent]]);
    }
